...
const calendar_edit_get = async (req, res) => {
  ...
  const venues = await Constant.find({church: global.churchId, category: 'Venue'},'_id category name value1').sort({ name: 1 });
  const access = await Constant.find({category: 'Calendar Access'}, '_id category name value1').sort({ name: 1});
  const status = await Constant.find({category: 'Status'}, '_id category name value1').sort({ sort: 1}); 
  await Calendar.findById(id)
    .populate('ministries','_id name')
    .then(result => {
      res.render('calendars/edit', {calendar: result, title: 'Edit calendar', ministries,venues, access, moment, status});
    })
    .catch(err => console.log(err));
}
...
.then(result => {
  ...
  result.startDate = calendar.startDate;
  result.endDate = calendar.endDate;
  result.agendaURL = calendar.agendaURL;
  result.description = calendar.description;
  result.venue = calendar.venue;
  result.access = calendar.access;
  result.ministries = [];
  if (calendar.ministry1 != 'None')
  {
    result.ministries.push(calendar.ministry1);
  }
  if (calendar.ministry2 != 'None')
  {
    result.ministries.push(calendar.ministry2);
  }
  if (calendar.ministry3 != 'None')
  {
    result.ministries.push(calendar.ministry3);
  }
